...
export class RegistroProfesorComponent implements OnInit {
  ...
  get numColegiatura() { return this.formulario.get('numColegiatura'); }
  ...
  getEspecialidadError(): string {
    if (this.especialidad?.hasError('required')) return 'La especialidad es obligatoria';
    if (this.especialidad?.hasError('minlength') || this.especialidad?.hasError('maxlength')) {
      return 'Debe tener entre 3 y 100 caracteres';
    }
    return '';
  }

  getNumColegiaturaError(): string {
    if (this.numColegiatura?.hasError('required')) return 'El número de colegiatura es obligatorio';
    if (this.numColegiatura?.hasError('minlength') || this.numColegiatura?.hasError('maxlength')) {
      return 'Debe tener entre 5 y 20 caracteres';
    }
    if (this.numColegiatura?.hasError('pattern')) return 'Solo se permiten letras y números';
    return '';
  }
  ...
  registrar(): void {
    ...
    this.profesorService.registerProfesor(payload).subscribe({
      next: (response) => {
        ...
        this.snackBar.open('¡Registro de profesor exitoso! Bienvenido a CampusFlow', 'Cerrar', {
          duration: 5000,
          horizontalPosition: 'end',
          verticalPosition: 'top',
  ...
  contactSupport(): void {
    this.snackBar.open('Redirigiendo a soporte académico...', 'Cerrar', {
      duration: 2000,
      horizontalPosition: 'end',
      verticalPosition: 'top'
    });
